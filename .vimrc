...
"                              ALE
" ALE only for linting/fixing; CoC handles LSP navigation/actions.
...
" Manual LSP formatting via CoC
nnoremap <silent> <leader>cf :call CocActionAsync('format')<CR>
xnoremap <silent> <leader>cf :<C-u>call CocActionAsync('formatSelected', visualmode())<CR>
...
"                     Cycle windows with F6 from terminal
tnoremap <silent> <F6>   <C-\><C-n><C-w>w
tnoremap <silent> <S-F6> <C-\><C-n><C-w>W
...
augroup StartAtTop
  autocmd!
  autocmd BufReadPost,BufWinEnter * if &buftype ==# '' | silent! normal! gg0 | endif
augroup END
